...
5. **读白文本须为纯文字**：表格中的「读白文本」以及各幕的「**读白**：xxx」中不要使用 Markdown（如 **、*、反引号`），否则会被语音合成逐字读出来。
6. 文件名格式：audio_001_幕名.wav（三位幕号），时长列留空。
7. 幕号从 1 开始连续编号，幕数按内容需要决定，不限制。
"""
...
3. 每幕 play_scene 内只调用一次 self.add_sound(str(self._audio_dir / audio_file))，且紧接 self.wait(duration)，不要在同一幕内重复调用 add_sound 或与动画重叠，否则会出现语音重音/叠音。
